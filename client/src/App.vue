...
export default {
  ...
  methods: {
    searchAlbumNames (query) {
      this.$store.dispatch('FILTER_ALBUMS_NAMES', query)
    },
    searchAlbums(query) {
      if (query) {
        //const payload = { 'url': `/api/search?term=${query}&entity=album&media=music`, 'query': query }
        const payload = { 'url': `http://localhost:5000/api/posts/${query}` , 'query': query }
